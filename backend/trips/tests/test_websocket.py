...
@database_sync_to_async
def create_trip(
    pick_up_address="123 abc road",
    drop_off_address="456 xyz road",
    status="REQUESTED",
    rider=None,
    driver=None,
):
...
class TestWebSocket:
    async def test_can_connect_to_server(self, settings):
        ...
        _, access = await create_user(username="test", password="testpassword")
    ...
    async def test_join_driver_pool(self, settings):
        ...
        _, access = await create_user(
            username="test", password="testpassword", group="driver"
        )
        communicator = WebsocketCommunicator(
            application=application, path=f"/taxi/?token={access}"
        )
    ...
    async def test_request_trip(self, settings):
        ...
        user, access = await create_user(
            username="test", password="testpassword", group="rider"
        )
        communicator = WebsocketCommunicator(
            application=application, path=f"/taxi/?token={access}"
        )
    ...
    async def test_driver_alerted_on_request(self, settings):
        ...
        # Create a rider user
        rider, rider_access = await create_user(
            username="rider", password="testpassword", group="rider"
        )

        communicator = WebsocketCommunicator(
            application=application, path=f"/taxi/?token={rider_access}"
        )
        connected, _ = await communicator.connect()
